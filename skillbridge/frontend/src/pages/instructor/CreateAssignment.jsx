...
import { ArrowLeft, Save, FileText, Calendar, AlertCircle } from 'lucide-react';
import api from '../../api';
...
export default function CreateAssignment() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/assignments/', { course_id: parseInt(form.course_id), title: form.title.trim(), description: form.description.trim()||null, due_date: form.due_date?new Date(form.due_date).toISOString():null });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      <div>
        <Link to="/instructor/grading" className="inline-flex items-center gap-2 text-sm text-[#8080a8] hover:text-white mb-4"><ArrowLeft size={13}/>Grading</Link>
      ...
      <form onSubmit={handleSubmit} className="glass rounded-2xl p-7 border border-white/[0.06] space-y-5">
        ...
        <div>
          <label className="block text-xs font-semibold text-[#7070a0] uppercase tracking-wider mb-2">Title *</label>
          <div className="relative"><FileText size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#7070a0]"/>
            <input type="text" value={form.title} onChange={e=>set('title',e.target.value)} placeholder="e.g. Week 3 Problems" className={`input-field pl-10 ${errors.title?'error':''}`}/>
          </div>
        ...
        <div>
          ...
          <textarea value={form.description} onChange={e=>set('description',e.target.value)} placeholder="What should students submit?" rows={3} className="input-field resize-none"/>
        </div>
        <div>
          <label className="block text-xs font-semibold text-[#7070a0] uppercase tracking-wider mb-2">Due Date</label>
          <div className="relative"><Calendar size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#7070a0]"/>
            <input type="datetime-local" value={form.due_date} onChange={e=>set('due_date',e.target.value)} className="input-field pl-10" style={{colorScheme:'dark'}}/>
          </div>
        </div>
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={loading} className="btn-emerald flex-1 py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
            {loading ? <LoadingSpinner size="sm"/> : <><Save size={14}/>Create</>}
          </button>
        </div>
      </form>
    </div>

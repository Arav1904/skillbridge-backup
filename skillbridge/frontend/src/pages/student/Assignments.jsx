...
import { Clock, CheckCircle, Send, AlertCircle, ClipboardList, Calendar, Trophy } from 'lucide-react';
import api from '../../api';
...
export default function Assignments() {
  ...
  const handleSubmit = async (assignmentId) => {
    if (!user?.user_id) { toast('Please sign in again', 'error'); return; }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-[#8080a8] text-sm mt-1">{pending.length} pending · {submitted.length} submitted</p>
      </div>
      ...
      {assignments.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center border border-white/[0.05]">
          <ClipboardList size={56} className="text-violet-400/30 mx-auto mb-6 animate-float" />
          <h2 className="text-2xl font-display font-bold text-white mb-3">No assignments yet</h2>
          <p className="text-[#8080a8]">Your instructor hasn't posted assignments yet.</p>
        </div>
      ) : (
          ...
          {pending.length > 0 && (
            <div>
              <h2 className="font-display font-semibold text-white text-lg mb-3 flex items-center gap-2">
                <AlertCircle size={16} className="text-amber-400"/> Pending
              </h2>
              <div className="space-y-3">
                {pending.map((a, i) => (
                  <div key={a.assignment_id} className="card p-5 border-l-[3px] border-l-amber-400/50 animate-fade-up" style={{animationDelay:`${i*40}ms`,animationFillMode:'both'}}>
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="badge badge-gold">Pending</span>
                          <span className="text-[10px] text-[#7070a0] font-mono">ID #{a.assignment_id}</span>
                        </div>
                        <h3 className="font-display font-semibold text-white text-base">{a.title}</h3>
                        {a.description && <p className="text-sm text-[#8080a8] mt-1 line-clamp-2">{a.description}</p>}
                        {a.due_date && (
                          <p className="text-xs text-amber-400/70 mt-2 flex items-center gap-1.5">
                            <Calendar size={11}/> Due {new Date(a.due_date).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'})}
                          </p>
                        )}
                      </div>
                      <button onClick={() => handleSubmit(a.assignment_id)}
            ...
            <div>
              ...
              <div className="space-y-3">
                {submitted.map((a, i) => {
                  ...
                  return (
                    <div key={a.assignment_id} className="card p-5 border-l-[3px] border-l-emerald-400/40 opacity-90">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1.5">
                            ...
                            {sub?.marks != null
                              ...
                              : <span className="badge badge-violet">Awaiting Grade</span>}
                          </div>
                          <h3 className="font-display font-semibold text-white text-base">{a.title}</h3>
                          {sub?.submission_date && (
                            <p className="text-xs text-[#7070a0] mt-1 flex items-center gap-1">
                              <CheckCircle size={10}/> Submitted {new Date(sub.submission_date).toLocaleDateString()}
                            </p>
                          )}
                        </div>
                        {sub?.marks != null && (
                          <div className="flex-shrink-0 text-center glass-emerald rounded-2xl px-6 py-3">
                            <div className={`text-3xl font-display font-bold ${sub.marks >= 80 ? 'text-emerald-400' : sub.marks >= 60 ? 'text-amber-400' : 'text-rose-400'}`}>
                              {sub.marks}
                            </div>
                            <div className="text-xs text-[#7070a0]">/ 100</div>
                          </div>
                        )}
                      </div>
                    </div>

...
from app.crud.certificate import create_certificate, get_user_certificates, get_all_certificates, delete_certificate
...
def my_certs(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return get_user_certificates(db, current_user.user_id)

@router.post("/", response_model=CertificateResponse, status_code=201)
def issue(data: CertificateCreate, db: Session = Depends(get_db), _=Depends(require_instructor)):
    return create_certificate(db, data.user_id, data.course_id, data.grade)

@router.delete("/{user_id}/{course_id}")
def revoke(user_id: int, course_id: int, db: Session = Depends(get_db), _=Depends(require_instructor)):
